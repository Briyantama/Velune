...
- **Legacy only (simplest):** `docker compose -f infra/docker-compose.yml up legacy-api redis` (Postgres depends on `DATABASE_URL`; start the `postgres` service separately if you want containerized Postgres)
