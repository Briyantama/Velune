// Notification service: budget alerts, recurring reminders, outbound email/push/webhooks (async).
package main
...
func main() {
	...
	defer log.Sync()
	cfg, err := sharedconfig.Load("notification-service")
	if err != nil {
	...
	r.Route("/api/v1", func(r chi.Router) {
		r.Get("/meta", func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"service":"notification-service","version":"0.1.0"}`))
		})
	})
