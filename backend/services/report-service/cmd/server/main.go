// Report service: read-side aggregations; consumes transaction/category APIs (HTTP) — no direct cross-DB access.
package main
...
func main() {
	...
	log.Info("upstream config",
		zap.String("transaction", cfg.TransactionServiceURL),
		zap.String("category", cfg.CategoryServiceURL),
	)
	r := chi.NewRouter()
	r.Use(middleware.RequestID, middleware.RealIP, middleware.Recoverer)
	r.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"status":"ok","service":"report-service"}`))
	})
	r.Route("/api/v1", func(r chi.Router) {
		r.Get("/meta", func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"service":"report-service","version":"0.1.0"}`))
		})
	})
