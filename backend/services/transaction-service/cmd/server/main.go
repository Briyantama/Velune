// Transaction service: income/expense ledger, accounts, transfers, recurring postings (bounded context).
package main
...
func main() {
	...
	cfg, err := sharedconfig.Load("transaction-service")
	if err != nil {
	...
	r.Get("/health", health("transaction-service"))
	r.Route("/api/v1", func(r chi.Router) {
		r.Get("/meta", meta("transaction-service"))
	})
...
func meta(name string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"service":"` + name + `","version":"0.1.0"}`))
	}
}
