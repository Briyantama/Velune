	...
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				return
			case <-ticker.C:
				if err := dispatchBudgetOutboxBatch(ctx, store, eventPublisher, "budget-service", cfg.OutboxBatchSize, cfg.OutboxMaxRetry, cfg.RetryBaseDelaySeconds, log); err != nil {
					log.Error("budget outbox dispatch failed", append(sharedlog.FieldsFromContext(ctx), zap.Error(err))...)
				}
			}
		}
	}()
		...
		go func() {
			...
			for {
				...
				case <-ticker.C:
					...
					if _, _, err := budgetrecon.VerifyBudgetAlertDrift(rctx, store.Pool, txSummaryClient, log); err != nil {
						log.Error("budget_reconcile", zap.Error(err))
					}
				}
			}
		}()
