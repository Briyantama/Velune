	...
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				return
			case <-ticker.C:
				if err := dispatchBudgetOutboxBatch(ctx, store, eventPublisher, cfg.OutboxBatchSize, cfg.OutboxMaxRetry, cfg.RetryBaseDelaySeconds, log); err != nil {
					log.Error("budget outbox dispatch failed", zap.Error(err))
				}
			}
		}
	}()
...
func dispatchBudgetOutboxBatch(
	ctx context.Context,
	...
	for rows.Next() {
		...
		if err := helper.ToJSONUnmarshal(payload, &env); err != nil {
		...
		if err := publisher.Publish(ctx, env); err != nil {
			if retryCount+1 >= maxRetry {
				...
				_, _ = store.Pool.Exec(ctx, `UPDATE event_outbox SET status='failed', retry_count=retry_count+1, updated_at=now() WHERE id=$1`, id)
				log.Error("budget outbox moved to dlq", zap.String("event_id", env.EventID.String()))
				continue
			}
			...
			_, _ = store.Pool.Exec(ctx, `
				...
				WHERE id=$1
			`, id, nextRetry)
			log.Warn("budget outbox retry", zap.String("event_id", env.EventID.String()), zap.Int("retry_count", retryCount+1))
			continue
		}
		_, _ = store.Pool.Exec(ctx, `UPDATE event_outbox SET status='sent', updated_at=now() WHERE id=$1`, id)
		log.Info("budget outbox published", zap.String("event_id", env.EventID.String()), zap.String("event_type", env.EventType))
	}
	return rows.Err()
}
