...
func (r *BudgetRepo) TransitionAlertStateAndEnqueue(ctx context.Context, budgetID uuid.UUID, usagePercent float64, envelopePayload json.RawMessage) (bool, error) {
	...
	qtx := r.s.Queries.WithTx(tx)
	prevState, err := qtx.BudgetAlertStateGetForUpdate(ctx, helper.ToPgUUID(budgetID))
	if err != nil {
	...
	if err := qtx.BudgetAlertStateUpsert(ctx, db.BudgetAlertStateUpsertParams{
		BudgetID:           helper.ToPgUUID(budgetID),
	...
	if emit {
		if err := qtx.OutboxInsert(ctx, db.OutboxInsertParams{
			...
			EventType: "notification.overspend.requested",
